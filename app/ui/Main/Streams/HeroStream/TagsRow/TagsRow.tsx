...
export default function TagsRow({ tags }: { tags: string[] }) {
  ...
  return (
    <div
      ref={containerRef}
      className="flex flex-1 gap-1 overflow-hidden flex-nowrap"
    >
      {tags.map((tag) => (
...
function Tag({ title }: { title: string }) {
  return (
    <div className="flex-none inline-flex justify-center items-center text-xs bg-black/50 text-text-2 px-1 rounded ">
      {title}
    </div>
  );
}
